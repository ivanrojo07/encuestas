<div>
    ...
    <div class="flex justify-end m-4">
        <button wire:click="$set('showModalForm',true)" class="bg-green-700 hover:bg-green-400 rounded-full py-1 px-2 text-white">
            Nueva Pregunta
        </button>
    </div>

    @forelse ($preguntas as $pregunta)
        <div class="p-2 card m-4 border border-gray-400 rounded-lg hover:shadow-md hover:border-opacity-0 transform hover:-translate-y-1 transition-all duration-200">
            <div class="grid grid-cols-3 gap-4">
                <div class="m-2">
                    <h2 class="text-xl font-medium">
                        {{$pregunta->pregunta}}
                    </h2>
                    <p>{{$pregunta->subtitle}} <span class="rounded-full bg-gray-400 text-white m-4 p-1">{{$pregunta->duracion}}</span> </p>
                </div>
                <div class="m-1">
                    <button wire:click="editPregunta({{$pregunta->id}})" class="bg-indigo-600 hover:bg-indigo-400 text-white rounded-full py-1 px-2">Editar</button>
                </div>
                <div class="m-1">
                    <button wire:click="deletePregunta({{$pregunta->id}})" class="bg-red-600 hover:bg-red-400 text-white rounded-full py-1 px-2">Eliminar</button>
                </div>
            </div>
    ...
    <x-jet-dialog-modal wire:model="showModalForm">
        ...
        <x-slot name="content">
            ...
            <div class="col-span-6 sm:col-span-4">
                ...
                <x-jet-input id="pregunta" type="text" class="mt-1 block w-full" wire:model.defer="pregunta" autofocus/>
            ...
            <div class="col-span-6 sm:col-span-4">
                ...
                <x-jet-input id="duracion" type="number" placeholder="Agregar los minutos de duración" step="1" class="mt-1 block w-full" wire:model.defer="duracion" />
